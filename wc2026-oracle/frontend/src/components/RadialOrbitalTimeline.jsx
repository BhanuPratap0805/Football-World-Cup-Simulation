...
export default function RadialOrbitalTimeline({ timelineData }) {
  ...
  const getStatusStyles = (status) => {
    switch (status) {
      case "completed":
        return "text-white bg-black border border-white";
      case "in-progress":
        return "text-black bg-white border border-black";
      case "pending":
        return "text-white bg-black/40 border border-white/50";
      default:
        return "text-white bg-black/40 border border-white/50";
    }
  };
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[1400px] h-[800px] flex items-center justify-center">
        ...
        >
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-purple-500 via-blue-500 to-teal-500 animate-pulse flex items-center justify-center z-10 shadow-[0_0_30px_rgba(59,130,246,0.5)]">
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    left: `-${(item.energy * 0.8 + 80 - 64) / 2}px`,
                    top: `-${(item.energy * 0.8 + 80 - 64) / 2}px`,
                  }}
                ></div>
                ...
                <div
                  className={`
                  absolute top-20 md:top-24 left-1/2 -translate-x-1/2 whitespace-nowrap
                  text-sm md:text-base font-semibold tracking-wider font-display uppercase
                  ...
                  <div className="absolute top-28 md:top-32 left-1/2 -translate-x-1/2 w-80 md:w-[400px] bg-black/80 backdrop-blur-xl border border-white/20 shadow-2xl shadow-white/5 overflow-visible rounded-2xl p-6">
                    ...
                    <div className="pb-3 border-b border-white/10 mb-3">
                      <div className="flex justify-between items-center mb-2">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider ${getStatusStyles(
                            item.status
                          )}`}
                        >
                      ...
                      <div className="mt-5 pt-4 border-t border-white/10">
                        <div className="flex justify-between items-center text-[10px] uppercase tracking-widest mb-2">
                          <span className="flex items-center text-white/80">
                            <Zap size={12} className="mr-1 text-teal-400" />
                            Impact Level
                          </span>
                        ...
                        <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-teal-400"
                          ...
                          <div className="flex flex-wrap gap-2">
                            {item.relatedIds.map((relatedId) => {
                              ...
                              return (
                                <button
                                  key={relatedId}
                                  className="flex items-center h-7 px-3 py-0 text-[10px] uppercase tracking-wider font-bold border border-white/10 bg-white/5 hover:bg-white/20 text-white/70 hover:text-white transition-all rounded-md"
                                  onClick={(e) => {
                                    e.stopPropagation();
